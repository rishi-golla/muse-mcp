...
SAMPLE_REPO_MARKER = ".muse-agent-proof"
...
def create_sample_retry_repo(repo_path: Path) -> Path:
    ...
    (repo_path / SAMPLE_REPO_MARKER).write_text(
        "generated sample repo for muse agent-loop proof\n",
        encoding="utf-8",
    )
...
async def _call_muse_plan(repo_signals: dict[str, list[str]]) -> dict[str, Any]:
    ...
    if not isinstance(structured_result, dict):
        raise RuntimeError("MCP muse_plan did not return structured output")
...
def run_agent_loop_proof(workspace: Path) -> dict[str, Any]:
    ...
    mcp_result = asyncio.run(_call_muse_plan(repo_signals))
