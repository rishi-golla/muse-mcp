...
def _run_case_variant(
    *,
    ...
    result = asyncio.run(_call_creative_plan(arguments))
...
async def _call_creative_plan(arguments: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(structured_result, dict):
        raise RuntimeError("creative_plan did not return structured output")
...
def _temporary_search_approval(enabled: bool):
    previous = os.environ.get("CREATIVITY_LAYER_LIVE_SEARCH_APPROVED")
    if enabled:
        os.environ["CREATIVITY_LAYER_LIVE_SEARCH_APPROVED"] = "1"
    try:
        yield
    finally:
        if previous is None:
            os.environ.pop("CREATIVITY_LAYER_LIVE_SEARCH_APPROVED", None)
        else:
            os.environ["CREATIVITY_LAYER_LIVE_SEARCH_APPROVED"] = previous
